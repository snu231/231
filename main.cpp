...
int main(int argc, char *argv[])
{
  ...
  insertTime = clock();
  insertionSort(insertArr, numData);
  insertTime = clock() - insertTime;
  check_result(insertArr, ansArr, numData);
  cout << "  Execution time : " << fixed << setprecision(5) << (float)insertTime/CLOCKS_PER_SEC << " seconds" << endl << endl;
